...
.text
	...
	sw $t7,8($s1)		#Armazena o valor de $t7 no endereço $s1+4
...
ISOS:	addi $t7,$zero,4	#Coloca o valor 4 no registrador $t7
	sw $t7,12($s1)		#Armazena o valor de $t7 no endereço $s1+8
	...
	sw $t7,16($s1)		#Armazena o valor de $t7 no endereço $s1+12
...
SRET:	addi $t7,$zero,6	#Coloca o valor 6 no registrador $t7
	sw $t7,20($s1)		#Armazena o valor de $t7 no endereço $s1+16
	j FIM			#A execução do código salta para linha indicada pelo label FIM
ELSE:	addi $t7,$zero,2	#Coloca o valor 2 no registrador $t7
	sw $t7,4($s1)		#Armazena o valor de $t7 no endereço $s1+20
FIM:
